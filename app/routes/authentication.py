...
def get_all_users():
    ...
    users = (
        KBUser.query.order_by(KBUser.email).all()
        if include_inactive
        else KBUser.query.filter(KBUser.active > 0)
        .order_by(KBUser.email)
        .all()
    )
...
def get_user_picture_by_id(user_id):
    ...
    temp_file = f'{tempfile.gettempdir()}/{str(uuid.uuid4()).replace("-", "")}'
    content = (
        user.picture
...
def update_user(user_id):
    ...
    if not current_user.admin and user.id != current_user.id:
        return abort(403)
...
def get_users_ownership():
    ...
    t = (
        tasks.Tasks.query.filter(tasks.Tasks.active > 0).all()
        if include_inactive
        else tasks.Tasks.query.all()
    )
